...
class TcpLogic(Tcp_ucpUi):
    ...
    def socket_open_tcps(self):
        ...
        try:
            ...
            # 关闭tcp socket
            self.socket_close()
            QMessageBox.critical(self, 'Error', 'Port ist bereits besetzt')
            ...
            # 当用户直接点击关闭按钮关闭客户端时，显示主机强制关闭的异常，否则服务器端会奔溃
                ...
                if recv_msg:
                    ...
                    self.statusbar_dict['rx'].setText(
                        'Anzahl receive：%s' % self.rx_count)
    ...
    def socket_open_tcpc(self):
        ...
        try:
            ...
            print("Error:", ret)
            QMessageBox.critical(self, 'Error', str(ret))
            self.socket_close()
        else:
            self.working = True
            ...
            connect_info = "Verbunden mit der Server-IP: %s Port: %s\n" % ip_port
        ...
        while True:
            try:
                ...
                if recv_msg:
                    ...
                    # 将接收到的数据字节数显示在状态栏的计数区域
                    self.rx_count += len(recv_msg)
                    self.statusbar_dict['rx'].setText('Anzahl erhalten:%s' % self.rx_count)
                else:
                    self.s.close()
                    msg = 'Die Leitung ist getrennt\n'
                    self.signal_write_msg.emit(msg)
                    self.working = False
                    self.socket_close()
                    break
    ...
    def data_send_t(self):
        ...
        if self.working is False:
            QMessageBox.critical(self, 'Achtung', 'Bitte richten Sie zuerst das TCP-Netzwerk ein')
        else:
            if self.link:
                ...
                if get_msg:
                    try:
                        ...
                        self.statusbar_dict['tx'].setText(
                            'Anzahl senden：%s' % self.tx_count)
                    except Exception as e_crst:
                        # QMessageBox.critical(self, '错误', '当前没有任何连接')
                        pass
                else:
                    QMessageBox.critical(self, 'Achtung', 'Senden darf nicht leer sein')

            else:
                QMessageBox.critical(self, 'Achtung', 'Derzeit keine Verbindung')
        ...
        if self.working is False:
            ...
            if self.link:
                ...
                if send_msg != b'':
                    ...
                    QMessageBox.critical(self, 'Achtung', 'Senden darf nicht leer sein')
                self.tx_count += len(send_msg)
                self.statusbar_dict['tx'].setText('Anzahl senden:%s' % self.tx_count)
            else:
                QMessageBox.critical(self, 'Achtung', 'Derzeit keine Verbindung')
